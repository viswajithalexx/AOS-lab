...
sst = sst.sel(lat = slice(5,22.5),lon = slice(45,65), time = slice('1994-01-01','2024-12-01'))
sss = sss.sel(lat = slice(5,22.5),lon = slice(45,65),time = slice('1994-01-01','2024-12-01'))
...
sst_annual = sst_reg.groupby('time.year').mean(dim='time')
...
plt.title('Obs-reconstructed vs Calculated Year-wise ΔpCO$_2$ (JAS-DJF) in NWIO (1994–2024)',fontweight ='bold',fontsize= 18)
plt.xlim(1995,2024)
plt.ylim(-5,50)
...
ax.set_title('Seasonal contrast between JAS and DJF of ΔpCO$_2$ in NWIO ',fontsize=25,fontweight = 'bold')
...
y_axis_offset = max(abs(np.array(means))) * -0.18  # small vertical offset from x-axis
