...
pco2_jfm = pco2_reg.resample(time='QS-JAN').mean(dim='time')  # Dec–Jan–Feb mean
pco2_jfm = pco2_jfm.isel(time=slice(1,-1))                 # Skip first incomplete DJF if needed
pco2_jfm = pco2_jfm.sel(time=pco2_jfm.time.dt.month == 1)

pco2_jfm_y = pco2_jfm.groupby('time.year').mean()
...
plt.title('Observed vs Calculated Year-wise pCO$_2$ Amplitude in NWIO (1994–2024) ')
plt.legend(frameon=False)
plt.grid(axis='y', linestyle='--', alpha=0.4)

plt.tight_layout()
plt.savefig("/home/bobco-08/24cl05012/CO2/plot/equation_pco2/valid_plots_eqn/nwio/obs_cal_nwio",dpi=500,bbox_inches="tight"
)
plt.show()
