...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 17 16:50:45 2026
...
lon =data['lon']
time = data['time']
dic = data['tco2'].sel(lat = slice(5,22.5),lon = slice(45,65))
alk = data['talk'].sel(lat = slice(5,22.5),lon = slice(45,65))
pco2 = data['spco2'].sel(lat = slice(5,22.5),lon = slice(45,65))
...
# why take annual mean of 31 years why not for each year seperate mean and do it ?
...
pco2_jfm = pco2_reg.sel(time=pco2_reg['time'].dt.month.isin([1,2,3]))

# Group by year
pco2_jfm_grouped = pco2_jfm.groupby('time.year')

pco2_jfm_y = pco2_jfm_grouped.mean()


# JJAS – DJF difference
delta_pco2 = (pco2_jas_y - pco2_jfm_y)
...
sst_jfm = sst_reg.sel(time=sst_reg['time'].dt.month.isin([1,2,3]))

# Group by year
sst_jfm_grouped = sst_jfm.groupby('time.year')

sst_jfm_y = sst_jfm_grouped.mean()
...
# sst_jfm_y = sst_jfm.groupby('time.year').mean()

delta_T = (sst_jas_y-sst_jfm_y)
...
dic_jfm = dic_reg.sel(time=dic_reg['time'].dt.month.isin([1,2,3]))

# Group by year and take mean over time
dic_jfm_grouped = dic_jfm.groupby('time.year')

dic_jfm_y = dic_jfm_grouped.mean()

# dic_jfm = dic_reg.resample(time='QS-JAN').mean(dim='time')  # Quarterly starting in Dec
# dic_jfm  = dic_jfm.isel(time=slice(1,-1))                 # Skip first incomplete DJF if needed
# dic_jfm  = dic_jfm.sel(time=dic_jfm.time.dt.month == 1)  

dic_jfm_y = dic_jfm.groupby('time.year').mean()

delta_dic = (dic_jas_y - dic_jfm_y)
...
alk_jfm = alk_reg.sel(time=alk_reg['time'].dt.month.isin([1,2,3]))

# Group by year and take mean over time
alk_jfm_grouped = alk_jfm.groupby('time.year')
alk_jfm_y = alk_jfm_grouped.mean()
...
sal_jfm = sal_reg.sel(time=sal_reg['time'].dt.month.isin([1,2,3]))

# Group by year and take mean over time
sal_jfm_grouped = sal_jfm.groupby('time.year')
sal_jfm_y = sal_jfm_grouped.mean()
# DJF using quarterly resample starting in December
...
dpco2_obs_nwio = delta_pco2.values
dpco2_cal_nwio = delta_pco2_cal.values
...
mask = dpco2_cal_nwio > dpco2_obs_nwio
for x, y in zip(years[mask], dpco2_cal_nwio[mask]):
    plt.vlines(x, ymin=0, ymax=y, color='red', linestyle='--', alpha=0.5)


plt.xlabel('Year')
plt.ylabel('ΔpCO$_2$ (µatm)')
plt.title('Observed vs Calculated Year-wise pCO$_2$ Amplitude (JAS-JFM) in NWIO (1994–2024)')
plt.xlim(1994,2024)
plt.ylim(0,50)
...
plt.xlabel('Year')
plt.ylabel('ΔpCO$_2$ (µatm)')
plt.title('Observed vs Calculated pCO$_2$ and Residual in NWIO(1994–2024)')
plt.legend(frameon=False, ncol=3)
plt.grid(True, linestyle='--', alpha=0.4)

plt.tight_layout()
plt.savefig(
    "/home/bobco-08/24cl05012/CO2/plot/equation_pco2/valid_plots_eqn/nwio/Observed vs Calculated Seasonal pCO$_2$ Amplitude and Residual_nwio",
    dpi=500,
    bbox_inches="tight"
)
plt.show()
...
plt.ylabel('ΔpCO$_2$ (µatm)')
plt.title('pCO$_2$ Decomposition (JulAS − JanFM)\nNorthwest Indian Ocean (5–22.5°N, 45–65°E)')
plt.legend(ncol=6, frameon=False)
plt.grid(axis='y', linestyle='--', alpha=0.4)

plt.tight_layout()
plt.savefig(
    "/home/bobco-08/24cl05012/CO2/plot/equation_pco2/valid_plots_eqn/nwio/nwio_drivers",
    dpi=500,
    bbox_inches="tight"
)
...
for i in peak_window:
    plt.text(months[i], values[i]-4, f'{values[i]:.0f}', ha='center', color='red')

for i in min_window:
    plt.text(months[i], values[i]+4, f'{values[i]:.0f}', ha='left', color='blue')
...
plt.title('Monthly Climatology of Surface pCO$_2$ in NWIO\n(Peak and Trough ±1 Month Highlighted)')
...
plt.savefig(
    "/home/bobco-08/24cl05012/CO2/plot/equation_pco2/valid_plots_eqn/nwio/mon_clim_nwio_peak_trough.png",
    dpi=500,
    bbox_inches="tight"
)
plt.show()
...
plt.xlabel('Year')
plt.ylabel('ΔpCO$_2$ (µatm)')
plt.title('Annual Time Series of pCO$_2$ and Drivers (JulAS − JanFM)\nNorthwest Indian Ocean (1994–2024)')
plt.legend(ncol=3, frameon=False)
plt.grid(True, linestyle='--', alpha=0.4)

plt.tight_layout()
plt.savefig(
    "/home/bobco-08/24cl05012/CO2/plot/equation_pco2/valid_plots_eqn/nwio/nwio_annual_timeseries_drivers.png",
