...
dic_nwio = data['tco2'].sel(lat = slice(5,22.5),lon = slice(45,65))
alk_nwio = data['talk'].sel(lat = slice(5,22.5),lon = slice(45,65))
pco2_nwio = data['spco2'].sel(lat = slice(5,22.5),lon = slice(45,65))
...
dic_eio = data['tco2'].sel(lat = slice(-6.5,5),lon = slice(49,92))
alk_eio = data['talk'].sel(lat = slice(-6.5,5),lon = slice(49,92))
pco2_eio = data['spco2'].sel(lat = slice(-6.5,5),lon = slice(49,92))
...
dic_nas = data['tco2'].sel(lat = slice(22.5,28),lon = slice(56,70))
alk_nas = data['talk'].sel(lat = slice(22.5,28),lon = slice(56,70))
pco2_nas = data['spco2'].sel(lat = slice(22.5,28),lon = slice(56,70))
...
dic_esio = data['tco2'].sel(lat = slice(-6.6,8),lon = slice(92,109))
alk_esio = data['talk'].sel(lat = slice(-6.6,8),lon = slice(92,109))
pco2_esio = data['spco2'].sel(lat = slice(-6.6,8),lon = slice(92,109))
...
# DIC
ax[0,0].scatter(dic_nwio_t, pco2_nwio_t, s=10)
ax[0,0].set_xlabel('DIC (µmol kg$^{-1}$)')
ax[0,0].set_ylabel('pCO$_2$ (µatm)')
ax[0,0].set_title(f'DIC vs pCO$_2$ (r = {r_dic_nwio:.2f})')

# ALK
ax[0,1].scatter(alk_nwio_t, pco2_nwio_t, s=10)
ax[0,1].set_xlabel('ALK (µmol kg$^{-1}$)')
ax[0,1].set_ylabel('pCO$_2$ (µatm)')
ax[0,1].set_title(f'ALK vs pCO$_2$ (r = {r_alk_nwio:.2f})')

# SST
ax[1,0].scatter(sst_nwio_t, pco2_nwio_t, s=10)
ax[1,0].set_xlabel('SST (°C)')
ax[1,0].set_ylabel('pCO$_2$ (µatm)')
ax[1,0].set_title(f'SST vs pCO$_2$ (r = {r_sst_nwio:.2f})')

# SSS
ax[1,1].scatter(sss_nwio_t, pco2_nwio_t, s=10)
ax[1,1].set_xlabel('SSS (psu)')
ax[1,1].set_ylabel('pCO$_2$ (µatm)')
ax[1,1].set_title(f'SSS vs pCO$_2$ (r = {r_sss_nwio:.2f})')
...
# Common title
fig.suptitle('Eastern Indian Ocean (EIO)', fontsize=16, fontweight='bold',y=0.98)
...
fig.suptitle('Eastern South Indian Ocean (ESIO)', fontsize=16, fontweight='bold',y = 0.95)
...
fig, ax = plt.subplots(2,2, figsize=(15,13),dpi = 400)
...
# -------- Create figure --------
fig, ax = plt.subplots(4, 4, figsize=(18,16), dpi=400)
...
for i, (name, dic, alk, sst, sss, pco2, r_dic, r_alk, r_sst, r_sss) in enumerate(regions):
    ...
    for j in range(4):
        ...
        ax[i, j].scatter(variables[j], pco2, s=8)
        ...
        if j == 0:
            ax[i, j].set_ylabel(f'{name}\npCO$_2$ (µatm)', fontsize=14)

        # Correlation title
        ax[i, j].set_title(f'r = {correlations[j]:.2f}', fontsize=12)

        # Tick style
        ax[i, j].tick_params(axis='both', labelsize=10, direction='in')
