...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  6 21:30:07 2025
...
da_mam_yearly_mean = da_mam.resample(mtime='Y').mean(dim='mtime')
mam_climatology = da_mam_yearly_mean.mean(dim='mtime')
mam_ano = da_mam_yearly_mean - mam_climatology
#%% detrending
import scipy.signal as sig
import time

mam_ano = mam_ano.fillna(0)
mam_detrend = sig.detrend(mam_ano, axis=0)
mam_detrend[mam_detrend == 0] = np.nan
mam_detrend = mam_detrend[2:-2]
#%% weightage
import eofs
from eofs.standard import Eof

n_time, n_lat, n_lon = mam_detrend.shape
eof_weights_lat = np.sqrt(np.cos(np.deg2rad(mam_ano['lat'].values)))
...
mam_2d = mam_detrend.reshape(n_time, n_lat * n_lon)
#%% solver
solver_mam = Eof(mam_2d, weights=weights_1d_space)
#%% eof maps
neofs = 3
covmaps = solver_mam.eofsAsCovariance(neofs=neofs)
#%% eof maps diff. modes
...
#%% variance
eofvar = solver_mam.varianceFraction(neigs=neofs) * 100
eofvar
#%% pcs
neofs = 3
pcs = solver_mam.pcs(pcscaling=1, npcs=neofs)
#%% eof1
...
plt.ylabel('Standardized', fontsize=12)
plt.xlabel('Year', fontsize=12)
plt.title(f'EOF 3:{season_name}:{eofvar[3]:.2f}%', fontsize=14)
plt.grid(True, linestyle=':', alpha=0.7)
plt.legend(loc='upper right')

plt.tight_layout()
plt.savefig('/home/bobco-08/24cl05012/CO2/plot/eof/{season_name}/IO_{season_name}_19592022_pcs3.pdf', dpi=500, bbox_inches='tight')
plt.show()
#%% combined
import numpy as np
import matplotlib.pyplot as plt
