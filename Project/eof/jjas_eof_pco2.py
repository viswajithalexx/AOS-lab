...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  6 21:40:47 2025
...
da_jjas_yearly_mean = da_jjas.resample(mtime='Y').mean(dim='mtime')
jjas_climatology = da_jjas_yearly_mean.mean(dim='mtime')
jjas_ano = da_jjas_yearly_mean - jjas_climatology
#%% detrending
import scipy.signal as sig
import time

jjas_ano = jjas_ano.fillna(0)
jjas_detrend = sig.detrend(jjas_ano, axis=0)
jjas_detrend[jjas_detrend == 0] = np.nan
jjas_detrend = jjas_detrend[2:-2]
#%% weightage
import eofs
from eofs.standard import Eof

n_time, n_lat, n_lon = jjas_detrend.shape
eof_weights_lat = np.sqrt(np.cos(np.deg2rad(jjas_ano['lat'].values)))
...
jjas_2d = jjas_detrend.reshape(n_time, n_lat * n_lon)
#%% solver
solver_jjas = Eof(jjas_2d, weights=weights_1d_space)
#%% eof maps
neofs = 3
covmaps = solver_jjas.eofsAsCovariance(neofs=neofs)
#%% eof maps diff. modes
...
#%% variance
eofvar = solver_jjas.varianceFraction(neigs=neofs) * 100
eofvar
#%% pcs
neofs = 3
pcs = solver_jjas.pcs(pcscaling=1, npcs=neofs)
#%% eof1
