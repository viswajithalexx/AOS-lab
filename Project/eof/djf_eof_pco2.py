...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  6 20:59:13 2025
...
da_djf = da.resample(mtime='QS-DEC').mean(dim =('mtime'))

da_djf = da_djf.isel(mtime=slice(1, None))

da_djf_year = da_djf.sel(mtime=da_djf .mtime.dt.month == 12)

djf_climatology = da_djf_year.mean(dim='mtime')

djf_ano = da_djf_year - djf_climatology
#%% detrending
...
djf_2d = djf_detrend.reshape(n_time, n_lat * n_lon)
#%% solver
solver_djf = Eof(djf_2d, weights=weights_1d_space)
#%% eof maps
neofs = 3
covmaps = solver_djf.eofsAsCovariance(neofs=neofs)
#%% eof maps diff. modes
...
#%% variance
eofvar = solver_djf.varianceFraction(neigs=neofs) * 100
eofvar
#%% pcs
neofs = 3
pcs = solver_djf.pcs(pcscaling=1, npcs=neofs)
#%% djf eof1
...
ax.set_ticks(fontsize = 25)
cbar.set_label('$\mu$atm',labelpad = 5, fontsize = 12)
...
plt.title(f'EOF 1:DJF:{eofvar[0]:.2f}%', fontsize=14)
...
plt.ylabel('Standardized', fontsize=12)
plt.xlabel('Year', fontsize=12)
plt.title(f'EOF 2:DJF:{eofvar[1]:.2f}%', fontsize=14)
plt.grid(True, linestyle=':', alpha=0.7)
plt.legend(loc='upper right')
...
plt.ylabel('Standardized', fontsize=12)
plt.xlabel('Year', fontsize=12)
plt.title(f'EOF 3:DJF:{eofvar[3]:.2f}%', fontsize=14)
plt.grid(True, linestyle=':', alpha=0.7)
plt.legend(loc='upper right')
